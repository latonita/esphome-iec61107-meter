...
enum IecCrcMethod : uint8_t { CRC_SUM7, CRC_XOR };

class Iec61107Component : public PollingComponent, public uart::UARTDevice {
 public:
  Iec61107Component() : tag_(generateTag()){};
  ...
  void set_programming_mode_required(bool req) { this->programming_mode_required_ = req; };
 ...
 protected:
  ...
  IecCrcMethod crc_method_{IecCrcMethod::CRC_SUM7};
  ...
  enum class State : uint8_t {
    ...
    OPEN_SESSION_GET_ID,
    SET_BAUD,
    ACK_READ_P0_CONFIRMATION,
    ACK_START_GET_INFO,
    PROGRAMMING_MODE_REQ,
    PROGRAMMING_MODE_ACK,
    MAIN_SESSION,
    GET_DATE,
    GET_TIME,
    CORRECT_TIME,
  ...
  void prepare_prog_password_frame_(const char *password);
  ...
  size_t receive_frame_ack_nak_();
  size_t receive_frame_ack_nak_close_();
