...
static const uint8_t CMD_ACK_SET_BAUD_AND_MODE[] = {ACK, '0', '5', '1', CR, LF};  // excat baud rate '5' set later
static uint8_t CMD_CLOSE_SESSION[] = {SOH, 0x42, 0x30, ETX, 0x75};                // crc gets recalculated in setup()
static const uint8_t CMD_CLOSE_SESSION_LEN = 5;
...
uint32_t byte_to_baud_rate(uint8_t baud_code) {
  baud_code -= '0';
  return (baud_code > 6) ? 300 : 300 * (1 << baud_code);
}
...
uint8_t Iec61107Component::calculate_crc_prog_frame_(uint8_t *data, size_t length, bool set_crc) {
  ...
  if (this->crc_method_ == IecCrcMethod::CRC_SUM7) {
    for (size_t i = 1; i < length - 1; i++) {
      crc = (crc + data[i]) & 0x7f;
    }
  } else if (this->crc_method_ == IecCrcMethod::CRC_XOR) {
    for (size_t i = 1; i < length - 1; i++) {
      crc ^= data[i];
    }
    crc &= 0x7f;
...
void Iec61107Component::prepare_prog_password_frame_(const char *password) {
  ...
  this->buffers_.amount_out =
      snprintf((char *) this->buffers_.out, MAX_OUT_BUF_SIZE, "%cP1%c(%s)%c\xFF", SOH, STX, password, ETX);
...
size_t Iec61107Component::receive_frame_ack_nak_close_() {
  ...
  auto frame_end_check_ack_nack_close = [this](uint8_t *b, size_t s) {
    auto ret = (s == 1 && (b[0] == ACK || b[0] == NAK)) ||
               (s == CMD_CLOSE_SESSION_LEN && b[0] == CMD_CLOSE_SESSION[0] && b[1] == CMD_CLOSE_SESSION[1] &&
                b[2] == CMD_CLOSE_SESSION[2] && b[3] == CMD_CLOSE_SESSION[3] && b[4] == CMD_CLOSE_SESSION[4]);
    if (ret) {
      if (b[0] == ACK) {
        ESP_LOGVV(TAG, "Frame ACK Stop");
      } else if (b[0] == NAK) {
