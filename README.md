...
- работа по оптопорту согласно протоколу: сначала 300 бод, затем переключение на скорость, запрошенную счетчиком (обычно 9600 бод), либо (не по стандарту) сразу работа на указанной скорости,
- работа по RS-485 проверена на скорости 9600 бод,
- работа с несколькими ПУ на одной шине RS-485,
- работа с несколькими шинами RS-485.

## 4. Принцип работы
Требуется подключение по оптопорту или RS-485 или CAN. Настраиваются общие параметры подключения к ПУ, а также набор сенсоров для считывания. Компонент поддерживает работу по протоколу МЭК 61107 в режиме C.
...
- оптопорт TTL-IR (не USB!) на 3.3 вольта
- **или** модуль трансивера RS-485,
  ...
  - A+ и B- на счетчике соединяем с модулем
...
### 5.1 Оптопорт. Самый простой вариант.
...
### 5.2 Рекомендуемый вариант подключения с RS-485 модулем с автоматическим выбором направления передачи 
...
### 5.3 Вариант подключения с классическим RS-485 модулем с 4 входами
...
└───────┘                    └─────────────┘                 └────────────────┘
```
### 5.4 Подключение нескольких счетчиков через RS-485
Подключение нескольких счетчиков возможно двумя способами:
- Подключение к одной шине RS-485 - самый простой вариант, просто необходимо параллельно подключить счетчики к одной шине: соединить А c А, B с B
```
     ┌─────┐    ┌─────┐         ┌─────┐
     │ # 1 │    │ # 2 │  ...    │ # N │
     │ A B │    │ A B │  ...    │ A B │
...
Если установлено несколько устройств на одной шине, необходимо **обязательно** указывать адрес счетчика.
...
sensor/text_sensor:
  - platform: iec61107
    # iec61107_id: meter_01   # если несколько счетчиков
...
Уточняем коды для запроса в документации на счетчик, либо из логов родной утилиты. [Примеры OBIS кодов из стандарта тут](obis_codes_electricity.md).
...
\
\
Производитель **Энергомера** для разных моделей использует разные кодировки, вместо OBIS кодов использует свои названия функций. Например, `ET0PE`.
...
Важный момент - при компиляции Esphome заменяет все не-латинские символы на `_`. А значит, если попадутся два сенсора с одинаковыми названиями с учетом замен - то Home Assistant будет смешивать их данные.
...
<details><summary>Для однофазного счетчика Нева 124 МТ, подключение по RS485</summary>
